...
def bpm_detector(data,fs):
    ...
    # Zaibo code
    offset = int(peak_ndx_adjusted[0])
    beat = max([(cD_sum[offset + i]*cD_sum[i],i) for i in xrange(0, len(cD_sum) - offset)])[1] * 16

    bpm = 60./ peak_ndx_adjusted * (fs/max_decimation)
    return bpm,correl,beat
...
if __name__ == '__main__':
    ...
    beats = numpy.zeros(max_window_ndx)
    ...
    for window_ndx in xrange(0,max_window_ndx):
        ...
        bpm, correl_temp, beat = bpm_detector(data,fs)
        if bpm == None:
            continue
        if len(bpm) > 1:
            continue
        bpms[window_ndx] = bpm
        beats[window_ndx] = beat
        correl = correl_temp
        
        #iterate at the end of the loop
        samps_ndx = samps_ndx+window_samps
    
    final_bpm = numpy.median(bpms)
    first_beat = 0
    for i in xrange(0, len(beats)):
        if abs(bpms[i]-final_bpm) < .1:
            first_beat = i*window_samps + beats[i]
            break
    ...
    f = open('cls_met.txt', 'a')
    f.write(str(bpm[0]) + '\t')
    f.write(str(first_beat) + '\t')
